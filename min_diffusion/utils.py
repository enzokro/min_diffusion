...
# %% auto 0
__all__ = ['get_initial_latents', 'text_embeddings', 'image_from_latents', 'show_image', 'image_grid', 'plot_schedules']
...
def get_initial_latents(seed, unet, height, width, device='cuda'):
    torch.manual_seed(seed)
    init_latents = torch.randn((1, unet.in_channels, height//8, width//8), dtype=unet.dtype, device=device)
    return init_latents

# %% ../nbs/01_utils.ipynb 4
def text_embeddings(prompts, tokenizer, text_encoder, device='cpu', maxlen=None):
...
def image_from_latents(latents, vae):
